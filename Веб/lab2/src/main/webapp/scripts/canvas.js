...
function drawGraph(){
    if (canvas.getContext) {
        ...
        ctx.lineTo(centerX, centerY - R/2); // Рисуем линию ко второй точке
        ...
        ctx.beginPath();
        ctx.moveTo(centerX - R, centerY); // Перемещаемся в первую точку
        ...
        ctx.arc(centerX, centerY, R/2, -2*Math.PI/2, -Math.PI/2);
        ctx.closePath(); // Замыкаем путь, чтобы создать круг
        ctx.fill(); // Заливаем круг указанным цветом
